...
function startLocalApi(userDataPath, requestedPort) {
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (url.pathname === '/api/auth/change-password' && req.method === 'POST') { if (!sessionToken || req.headers.authorization !== `Bearer ${sessionToken}`) return json(res, 401, { error: 'Authentication required' }); const input = await body(req); const current = String(input.currentPassword || ''); const next = String(input.newPassword || ''); const record = db.prepare("SELECT value FROM app_meta WHERE key = 'admin_password'").get(); if (!record || !verifyPassword(current, record.value)) return json(res, 401, { error: 'Current password is incorrect' }); if (next.length < 10) return json(res, 400, { error: 'New password must be at least 10 characters' }); db.prepare("UPDATE app_meta SET value = ? WHERE key = 'admin_password'").run(hashPassword(next)); return json(res, 200, { ok: true }); }

...
const collections = {
  '/api/students': { table: 'students', fields: ['full_name', 'full_name_arabic', 'student_number', 'national_id', 'grade', 'class_name', 'guardian_name', 'guardian_phone', 'enrollment_date', 'status', 'academic_year_id'] },
  '/api/teachers': { table: 'teachers', fields: ['full_name', 'full_name_arabic', 'name', 'surname', 'username', 'password', 'english_name', 'employee_code', 'national_id', 'nationality', 'gender', 'marital_status', 'religion', 'phone', 'email', 'address', 'area', 'country', 'height', 'weight', 'branch', 'academic_level', 'subject', 'weekly_classes', 'is_employee', 'status', 'academic_year_id'] },
...
function toDatabaseInput(table, input) {
  ...
  if (table === 'teachers') return { full_name: input.fullName || [input.name, input.surname].filter(Boolean).join(' '), full_name_arabic: input.fullNameArabic, name: input.name, surname: input.surname, username: input.username, password: input.password, english_name: input.englishName, employee_code: input.employeeCode, national_id: input.nationalId, nationality: input.nationality, gender: input.gender, marital_status: input.maritalStatus, religion: input.religion, phone: input.phone, email: input.email, address: input.address, area: input.area, country: input.country, height: input.height, weight: input.weight, branch: input.branch, academic_level: input.academicLevel, subject: input.subject, weekly_classes: input.weeklyClasses, is_employee: input.isEmployee === undefined ? undefined : (input.isEmployee ? 1 : 0), status: input.status, academic_year_id: input.academicYearId ?? 1 };
...
function startLocalApi(userDataPath, requestedPort) {
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (url.pathname === '/api/auth/status') return json(res, 200, { setupRequired: !db.prepare("SELECT value FROM app_meta WHERE key = 'admin_password'").get() || !db.prepare("SELECT value FROM app_meta WHERE key = 'admin_username'").get(), authenticated: Boolean(sessionToken) });
      if (url.pathname === '/api/auth/setup' && req.method === 'POST') { const input = await body(req); const existingPassword = db.prepare("SELECT value FROM app_meta WHERE key = 'admin_password'").get(); const existingUsername = db.prepare("SELECT value FROM app_meta WHERE key = 'admin_username'").get(); if (existingPassword && existingUsername) return json(res, 409, { error: 'Admin already configured' }); const username = String(input.username || '').trim(); if (!username) return json(res, 400, { error: 'Username is required' }); if (!input.password || String(input.password).length < 10) return json(res, 400, { error: 'Password must be at least 10 characters' }); db.prepare("INSERT OR REPLACE INTO app_meta (key,value) VALUES ('admin_username', ?), ('admin_password', ?)").run(username, hashPassword(String(input.password))); return json(res, 201, { ok: true }); }
      if (url.pathname === '/api/auth/login' && req.method === 'POST') { const input = await body(req); const usernameRecord = db.prepare("SELECT value FROM app_meta WHERE key = 'admin_username'").get(); const username = usernameRecord ? usernameRecord.value : 'admin'; const record = db.prepare("SELECT value FROM app_meta WHERE key = 'admin_password'").get(); if (String(input.username || '').trim() !== username || !record || !verifyPassword(String(input.password || ''), record.value)) return json(res, 401, { error: 'Invalid username or password' }); sessionToken = crypto.randomBytes(32).toString('hex'); return json(res, 200, { token: sessionToken }); }
      if (url.pathname === '/api/auth/change-password' && req.method === 'POST') { if (!sessionToken || req.headers.authorization !== `Bearer ${sessionToken}`) return json(res, 401, { error: 'Authentication required' }); const input = await body(req); const current = String(input.currentPassword || ''); const nextUsername = String(input.newUsername || '').trim(); const next = String(input.newPassword || ''); const record = db.prepare("SELECT value FROM app_meta WHERE key = 'admin_password'").get(); if (!record || !verifyPassword(current, record.value)) return json(res, 401, { error: 'Current password is incorrect' }); if (!nextUsername) return json(res, 400, { error: 'New username is required' }); if (next.length < 10) return json(res, 400, { error: 'New password must be at least 10 characters' }); db.prepare("INSERT OR REPLACE INTO app_meta (key,value) VALUES ('admin_username', ?)").run(nextUsername); db.prepare("UPDATE app_meta SET value = ? WHERE key = 'admin_password'").run(hashPassword(next)); return json(res, 200, { ok: true }); }
      ...
      if (url.pathname === '/api/library/borrows' && req.method === 'POST') {
        ...
        const student = db.prepare('SELECT id FROM students WHERE id = ?').get(Number(input.studentId ?? input.borrowerId));
        if (!book || !student) return json(res, 404, { error: 'Book or student not found' });
        const existingBorrow = db.prepare('SELECT id FROM borrows WHERE book_id = ? AND student_id = ? AND returned_at IS NULL').get(book.id, student.id);
        if (existingBorrow) return json(res, 409, { error: 'This borrower already has an active loan for this book' });

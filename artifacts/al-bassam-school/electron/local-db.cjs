...
function openSchoolDatabase(userDataPath) {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS books (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT NOT NULL, author TEXT NOT NULL DEFAULT '', isbn TEXT NOT NULL DEFAULT '', category TEXT NOT NULL DEFAULT '', language TEXT NOT NULL DEFAULT 'Arabic', volume TEXT NOT NULL DEFAULT '', copies INTEGER NOT NULL DEFAULT 1, available_copies INTEGER NOT NULL DEFAULT 1, total_copies INTEGER NOT NULL DEFAULT 1, date_added TEXT NOT NULL DEFAULT '', deposit_number TEXT NOT NULL DEFAULT '', status TEXT NOT NULL DEFAULT 'available', publication_place TEXT NOT NULL DEFAULT '', publication_date TEXT NOT NULL DEFAULT '', general_number TEXT NOT NULL DEFAULT '', special_number TEXT NOT NULL DEFAULT '', description TEXT NOT NULL DEFAULT '', cover_image TEXT NOT NULL DEFAULT '', shelf TEXT NOT NULL DEFAULT '');
    ...
    CREATE TABLE IF NOT EXISTS borrows (id INTEGER PRIMARY KEY AUTOINCREMENT, book_id INTEGER NOT NULL, student_id INTEGER NOT NULL, borrowed_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, due_date TEXT, returned_at TEXT);
  ...
  if (db.prepare('SELECT COUNT(*) AS count FROM students').get().count === 0) {
    const seed = db.transaction(() => {
      ...
      const book = db.prepare('INSERT INTO books (title, author, isbn, category, language, copies, total_copies, available_copies, shelf, date_added) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
      book.run('Our Planet', 'David Attenborough', '9780521536608', 'Science', 'English', 5, 5, 4, 'A-12', '2025-09-01');
      book.run('Complete ICT IGCSE', 'Paul Culling', '9780981775470', 'Technology', 'English', 8, 8, 6, 'B-04', '2025-09-01');
      book.run('الرياضيات للمرحلة المتوسطة', 'وزارة التعليم', '9789953456789', 'Mathematics', 'Arabic', 12, 12, 10, 'D-01', '2025-09-01');
